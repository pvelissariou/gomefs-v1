...
!
      ...
      integer :: bufZONE, nCELLS, IS_BEG, IN_BEG

      ! The number of extra points used to expand the buffer zone
      ! internally in the ROMS domain.
      integer, parameter :: my_XTRAPNTS = 0

      ! The number of ghost (halo) points used in ROMS, can be 2 or 3
      ! depending upon the advection numerical scheme used in ROMS.
      integer, parameter :: my_GHSTPNTS = GHOST_POINTS
...
!
      ...
      ! Need to take my_GHSTPNTS cells out from the number of nCELLS of the
      ! buffer zone width because the calculations below are performed
      ! based on the interior grid points:
      !   (Lm, Mm) = (x_rho - my_GHSTPNTS, eta_rho - my_GHSTPNTS)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      nCELLS = bufZONE + my_XTRAPNTS - my_GHSTPNTS

      ! We use 1s to produce a value cff1 = 1.0 at the first boundary
      ! grid point. Below we adjust cff1 to the value for the first
      ! grid point of the internal ROMS calculations.
      ! (minus the GHOST points).
      rtim1 =  1.0_r8 / 86400.0_r8  ! For 1 sec
!      rtim1 =  0.5_r8
!      rtim2 =  10.0_r8              ! For 10 days
      rtim2 = 360.0_r8              ! For 360 days

      cff1 = 1.0_r8 / (rtim1 * 86400.0_r8)
      cff2 = 1.0_r8 / (rtim2 * 86400.0_r8)
      cff3 = log(cff1 / cff2)

      ! Re-calculate cff1 and cff3 based upon the number of my_GHSTPNTS
      IF (my_GHSTPNTS .gt. 1) THEN
        rratio = REAL(my_GHSTPNTS - 1, r8) / REAL(my_GHSTPNTS, r8)
        cff1 = cff1 * exp( - rratio * cff3 )
        cff3 = log(cff1 / cff2)
      ENDIF
